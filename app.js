...
function getXML(url, jsonTrackId, resultsHTML) {
  var resultsDiv = document.getElementById("resultsDiv");
  $.when($.get(url, function(data) {})).then(function(data) {
    console.log(data);
...
function displayEpisodes(eps, jsonTrackId, epPic) {
  ...
  for (var i = 0; i < 20; i++) {
    ...
    var dateRaw = $(eps[i]).find("pubDate")["0"].textContent;
    var pubdate = dateRaw.slice(0, 16);
...
function showPodcastDeets(id) {
  ...
  $.when($.get(getXMLParameters, function(data) {})).then(function(data) {
    // console.log(data);
    var podcastDescript = $(data).find("channel > description");
    var episodes = $(data).find("item");
